...
const ensureDatabase = (): Promise<IDBDatabase> => {
  return new Promise((resolve, reject) => {
    ...
    request.onupgradeneeded = (event: IDBVersionChangeEvent) => {
      ...
      // 删除旧版本的对象存储（如果存在）
      if (!db.objectStoreNames.contains(dbStoreNames.PLAYLISTS)) {
        // 创建歌单对象存储
      ...
      if (!db.objectStoreNames.contains(dbStoreNames.SONGS)) {
        // 创建歌曲对象存储
    ...
    request.onerror = (event: Event) => {
      ...
      console.error("IndexedDB 初始化失败", error); // 输出错误日志
      reject(error);
    };
  });
};
...
export const addPlaylist = (
  ...
  ensureDatabase()
    .then((db) => {
      const transaction = db.transaction(dbStoreNames.PLAYLISTS, "readwrite"); // 创建读写事务
      const store = transaction.objectStore(dbStoreNames.PLAYLISTS); // 获取对象存储
      const addRequest = store.add(playlist); // 添加歌单到对象存储

      // 添加成功时触发
      addRequest.onsuccess = () => {
        console.log("歌单添加成功"); // 输出成功日志
        callback({ id: addRequest.result as number, success: true }); // 调用回调函数
      };

      // 添加失败时触发
      addRequest.onerror = (event: Event) => {
        console.error("歌单添加失败", (event.target as IDBRequest).error); // 输出错误日志
        callback({ id: null, success: false }); // 调用回调函数
      };
    })
    .catch((error) => {
...
export const getAllPlaylists = (
  callback: (result: GetAllPlaylistsCallback) => void
): void => {
  ensureDatabase()
    .then((db) => {
      const transaction = db.transaction(dbStoreNames.PLAYLISTS, "readonly"); // 创建只读事务
      const store = transaction.objectStore(dbStoreNames.PLAYLISTS); // 获取对象存储
      const playlists: Playlist[] = []; // 存储所有歌单
      ...
      store.openCursor().onsuccess = (event: Event) => {
        ...
        if (cursor) {
          ...
          callback({ success: true, playlists }); // 遍历完成后调用回调函数
        }
      };

      transaction.onerror = (event: Event) => {
        console.error("获取歌单失败", (event.target as IDBTransaction).error);
...
export const deletePlaylistById = (
  ...
  ensureDatabase()
    .then((db) => {
      ...
      const transaction = db.transaction(dbStoreNames.PLAYLISTS, "readwrite");
...
export const addSong = (
  ...
  ensureDatabase()
    .then((db) => {
      const transaction = db.transaction(dbStoreNames.SONGS, "readwrite");
...
export const getAllSongs = (
  callback: (result: GetAllSongsCallback) => void
): void => {
  ensureDatabase()
    .then((db) => {
      const transaction = db.transaction(dbStoreNames.SONGS, "readonly");
      const store = transaction.objectStore(dbStoreNames.SONGS);
      const songs: Song[] = [];
...
export const getPlaylistById = (
  ...
  ensureDatabase()
    .then((db) => {
      ...
      const transaction = db.transaction(dbStoreNames.PLAYLISTS, "readonly");
      const store = transaction.objectStore(dbStoreNames.PLAYLISTS);
...
export const importData = (
  data: { songs: Song[]; playlists: Playlist[] },
  callback: (result: SimpleCallback) => void
): void => {
  ensureDatabase()
    .then((db) => {
      const songTransaction = db.transaction(dbStoreNames.SONGS, "readwrite");
      ...
      for (const song of data.songs.filter((song) => song.id !== undefined)) {
        ...
        addRequest.onsuccess = (event: Event) => {
          songIdMap[song.id as number] = (event.target as IDBRequest<number>)
            .result as number;
        };
      }
      ...
      songTransaction.oncomplete = () => {
        ...
        const playlistStore = playlistTransaction.objectStore(
          dbStoreNames.PLAYLISTS
        );
